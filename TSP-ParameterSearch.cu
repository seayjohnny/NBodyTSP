...
__global__ void accelerations(float2* nodes, float2* pos, float2* vel, float2* acc, float *mass, float innerRadius, float outerRadius, int n)
{
    float2 force;
    float2 node, nodePos;
    float dx, dy, dist, edgeLength;
...
int main(int argc, char** argv)
{
	...
	normalizingFactor = linearScalePoints(nodes, numberOfNodes, 0.75);
